...
(function (root, factory) {
    ...
    ko.bindingHandlers.listSlider = {
        ...
        update: function (element, valueAccessor, allBindings, viewModel, context) {
            ...
            var $element = $(element);
            // Not all browsers support this the same way unfortunately
            if ('onwheel' in window) {
                // Modern (FF17+, IE9+)
                $element.on('wheel', function (event) {
                    $element.parent().scrollLeft($element.parent().scrollLeft() + event.originalEvent.deltaY * 40);
                });
            } else if ('onmousewheel' in window) {
                // Legacy (Webkit - https://bugs.webkit.org/show_bug.cgi?id=94081)
